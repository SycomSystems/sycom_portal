...
files['src/app/api/stock/items/route.ts'] = """import { NextRequest, NextResponse } from 'next/server'
...
files['src/app/api/stock/suppliers/route.ts'] = """import { NextRequest, NextResponse } from 'next/server'
import { getServerSession } from 'next-auth'
import { authOptions } from '@/lib/auth'
import { prisma } from '@/lib/prisma'
...
files['src/app/api/stock/movements/route.ts'] = """import { NextRequest, NextResponse } from 'next/server'
import { getServerSession } from 'next-auth'
import { authOptions } from '@/lib/auth'
import { prisma } from '@/lib/prisma'
...
files['src/app/(admin)/admin/sklad/page.tsx'] = r"""'use client'
...
print('Done! Now run: git add -A && git commit -m "Add Sklad feature" && git push')
print('Then: npx prisma db push && npx prisma generate && npm run build && pm2 restart sycom-portal')
