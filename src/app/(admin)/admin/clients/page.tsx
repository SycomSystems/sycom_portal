...
const HOURS_LABELS: Record<HoursType, string> = {
  STANDARD: 'Standard',
  STANDARD_MIMO: 'Standard mimo prac. času',
  SERVER: 'Server',
  SERVER_MIMO: 'Server mimo prac. času',
}
...
const emptyPricing = (): PricingMap =>
  Object.fromEntries(HOURS_TYPES.map(t => [t, ''])) as PricingMap
          ...
          <div key={type} className="flex items-center gap-2 bg-gray-50 border border-gray-200 rounded-xl px-3 py-2">
            ...
            <div className="relative w-24 flex-shrink-0">
              <input
                ...
                placeholder="0.00"
                className="w-full pr-6 pl-2 py-1 border border-gray-200 rounded-lg text-sm text-right focus:outline-none focus:border-sycom-400 focus:ring-2 focus:ring-sycom-100 bg-white"
              />
...
function FormFields({ form, setForm }: { form: FormState; setForm: (f: FormState) => void }) {
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
      <div>
        <label className="block text-xs font-medium text-gray-500 mb-1">Názov klienta *</label>
      ...
      <div>
        <label className="block text-xs font-medium text-gray-500 mb-1">Kontaktná osoba</label>
      ...
      <div>
        <label className="block text-xs font-medium text-gray-500 mb-1">Telefón</label>
        <input type="text" value={form.phone} onChange={e => setForm({ ...form, phone: e.target.value })}
          placeholder="+421 900 000 000"
          className="w-full px-3 py-2 border border-gray-200 rounded-xl text-sm focus:outline-none focus:border-sycom-400 focus:ring-2 focus:ring-sycom-100" />
      </div>
      <div>
        <label className="block text-xs font-medium text-gray-500 mb-1">IČO</label>
        <input type="text" value={form.ico} onChange={e => setForm({ ...form, ico: e.target.value })}
          className="w-full px-3 py-2 border border-gray-200 rounded-xl text-sm focus:outline-none focus:border-sycom-400 focus:ring-2 focus:ring-sycom-100" />
      </div>
      <div>
        <label className="block text-xs font-medium text-gray-500 mb-1">DIČ</label>
        <input type="text" value={form.dic} onChange={e => setForm({ ...form, dic: e.target.value })}
          className="w-full px-3 py-2 border border-gray-200 rounded-xl text-sm focus:outline-none focus:border-sycom-400 focus:ring-2 focus:ring-sycom-100" />
      </div>
      <div>
        <label className="block text-xs font-medium text-gray-500 mb-1">DIČ DPH</label>
      ...
      <div>
        ...
        <input type="text" value={form.www} onChange={e => setForm({ ...form, www: e.target.value })}
      ...
      <div className="sm:col-span-2">
        ...
        <textarea value={form.notes} onChange={e => setForm({ ...form, notes: e.target.value })}
          rows={2} className="w-full px-3 py-2 border border-gray-200 rounded-xl text-sm focus:outline-none focus:border-sycom-400 focus:ring-2 focus:ring-sycom-100 resize-none" />
      </div>
...
export default function ClientsPage() {
  ...
  const handleAdd = async () => {
    ...
    try {
      const res = await fetch('/api/clients', {
        ...
        body: JSON.stringify({ ...newForm, pricing: pricingForApi(newForm.pricing) }),
      })
  ...
  const handleEdit = async (id: string) => {
    ...
    try {
      const res = await fetch(`/api/clients/${id}`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ ...editForm, pricing: pricingForApi(editForm.pricing) }),
      })
      const data = await res.json()
      if (!res.ok) throw new Error(data.error || 'Chyba')
      setEditId(null)
      load()
      showStatus('success', `Klient "${data.name}" bol upravený.`)
    } catch (e: any) {
      showStatus('error', e.message)
    }
  }
  ...
  const startEdit = (client: Client) => {
    ...
    setEditForm({
      ...
      ico: client.ico ?? '',
      dic: client.dic ?? '',
      dicDph: client.dicDph ?? '',
      address: client.address ?? '',
      www: client.www ?? '',
      notes: client.notes ?? '',
      pricing: pricingToMap(client.pricing),
    })
    ...
    <PortalLayout>
      <div className="max-w-4xl mx-auto py-8 px-6">
        <div className="mb-8">
          ...
          <p className="text-sm text-gray-500 mt-1">Spravujte zoznam klientov a ich cenníky.</p>
        </div>
          ...
          <div className="p-6 space-y-4">
            ...
            <div className="flex justify-end">
              <button onClick={handleAdd} disabled={adding || !newForm.name.trim()}
                ...
                <Plus size={15} />
                {adding ? 'Pridávam...' : 'Pridať klienta'}
              </button>
            </div>
            {status && (
              <div className={`flex items-start gap-2.5 px-4 py-3 rounded-xl text-sm ${status.type === 'success' ? 'bg-green-50 border border-green-200 text-green-800' : 'bg-red-50 border border-red-200 text-red-800'}`}>
                  ...
                  <div className="px-6 py-4 flex items-center gap-4">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-xs text-gray-400 flex items-center gap-2 mt-0.5 flex-wrap">
                        <span className="flex items-center gap-1"><Users size={11} /> {client._count.users} použ.</span>
                        ...
                        {HOURS_TYPES.filter(t => priceMap[t] > 0).length > 0 && (
                          <span className="flex items-center gap-1 text-sycom-600">
                            <Euro size={10} />
                            {HOURS_TYPES.filter(t => priceMap[t] > 0).map(t => priceMap[t].toFixed(2)).join(' / ')}
                          </span>
                        )}
                      </p>
                    </div>
                    <div className="flex items-center gap-1 ml-auto">
                      <button onClick={() => setExpandedId(expandedId === client.id ? null : client.id)}
                        className="p-2 text-gray-400 hover:text-gray-600 hover:bg-gray-50 rounded-lg transition-colors" title="Detail">
                        {expandedId === client.id ? <ChevronUp size={14} /> : <ChevronDown size={14} />}
                      </button>
                      <button onClick={() => startEdit(client)}
                        className="p-2 text-gray-400 hover:text-sycom-500 hover:bg-sycom-50 rounded-lg transition-colors" title="Upraviť">
                        <Pencil size={14} />
                      </button>
                      <button onClick={() => handleDelete(client.id, client.name)}
                        className="p-2 text-gray-400 hover:text-red-500 hover:bg-red-50 rounded-lg transition-colors" title="Vymazať">
                        <Trash2 size={14} />
                      </button>
                    </div>
                  </div>
                      ...
                      {editId === client.id ? (
                        ...
                        <div className="pt-4 space-y-4">
                          <div className="grid grid-cols-2 sm:grid-cols-3 gap-x-6 gap-y-3">
                            {([['IČO', client.ico], ['DIČ', client.dic], ['DIČ DPH', client.dicDph], ['Telefón', client.phone], ['WWW', client.www], ['Adresa', client.address]] as [string, string | null][])
...
export default function ClientsPage() {
  ...
  const handleDelete = async (id: string, name: string) => {
    if (!confirm('Naozaj chcete vymazat klienta "' + name + '"? Pouzivatelia nebudu vymazani, iba sa odpoji ich priradenie.')) return
    try {
      const res = await fetch('/api/clients/' + id, { method: 'DELETE' })
                ...
                <div key={client.id}>
                  <div className="px-6 py-4 flex items-center gap-4">
                    ...
                    <div className="flex items-center gap-1 ml-auto">
                      ...
                      <button onClick={() => startEdit(client)}
                        className="p-2 text-gray-400 hover:text-sycom-500 hover:bg-sycom-50 rounded-lg transition-colors" title="Upravit">
                        <Pencil size={14} />
                      </button>
                      <button onClick={() => handleDelete(client.id, client.name)}
                        className="p-2 text-gray-400 hover:text-red-500 hover:bg-red-50 rounded-lg transition-colors" title="Vymazat">
                        <Trash2 size={14} />
                      </button>
                    </div>
                  </div>
