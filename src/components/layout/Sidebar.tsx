...
export function Sidebar() {
  ...
  useEffect(() => {
    fetch('/api/settings/phone')
      .then(r => r.json())
      .then(data => { if (data.phone) setHelpdeskPhone(data.phone) })
      .catch(() => {})
  }, [])
  ...
  return (
    <aside className="w-[220px] flex-shrink-0 bg-white border-r border-gray-200 fixed top-[62px] bottom-0 left-0 flex flex-col">
      ...
      {/* Helpdesk card */}
      <div className="m-3 p-3 bg-sycom-50 border border-sycom-200 rounded-xl text-center">
        <Phone size={16} className="mx-auto mb-1 text-sycom-500" />
        <p className="text-[11px] font-bold text-sycom-600 mb-0.5">Helpdesk linka</p>
        <p className="font-mono text-xs text-gray-700">{helpdeskPhone}</p>
      </div>
    </aside>
  )
}
